@extends('layouts.app')
@section('title')
    Dashboard - Hotlist Product Manager
@endsection
@section('jsfiles')
<script src="{{asset('js/datatables.min.js')}}"></script>
@endsection
@section('header')
    Manage Hotlist Products
@endsection
@section('content')
                        ...
                        <table id="add-row" class="display nowrap table table-striped table-hover" cellspacing="0" width="100%">
                            ...
                            <tbody>
                              ...
                              <tr>
                               ...
                               <td>
                               <a  class="btn btn-info btn-sm" href="{{route('product.viewhotlistDetails', ['slug'=>$product->slug])}}">
                                    <i class="fa fa-eye" aria-hidden="true"></i> Details 
                                </a>
                               </td>
                               <td>
                                <form action="{{route('hotlist_product.approve')}}" method="post">
                                    @csrf
                               ...
                               <td>
                                <form action="{{route('hotlist_product.reject')}}" method="post">
                                    @csrf
                                    <input type="hidden" name="id" value="{{$product->id}}">
