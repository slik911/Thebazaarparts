@extends('layouts.app')
@section('title')
    Dashboard - Featured Product Manager
@endsection
@section('jsfiles')
                        ...
                        <table id="add-row" class="display nowrap table table-striped table-hover" cellspacing="0" width="100%">
                            ...
                            <tbody>
                              ...
                              <tr>
                               ...
                               <td>
                               <a  class="btn btn-info btn-sm" href="{{route('product.viewFeaturedDetails', ['slug'=>$product->slug])}}">
                                    <i class="fa fa-eye" aria-hidden="true"></i> Details 
                                </a>
                               </td>
                               <td>
                                <form action="{{route('featured_product.approve')}}" method="post">
                                    @csrf
                               ...
                               <td>
                                <form action="{{route('featured_product.reject')}}" method="post">
                                    @csrf
                                    <input type="hidden" name="id" value="{{$product->id}}">
